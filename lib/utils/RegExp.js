...
var decimalReg = /^([1-9][0-9]*)(\.[\d]{1,2})?|(0\.[\d]{1,2})$/; // 请输入大于0的整数或两位小数
...
var rules = {
  uName: function uName(txt) {
    ...
    if (isRequired) {
      reg.unshift({
        required: true,
        message: "\u8BF7\u8F93\u5165".concat(txt)
      });
    }

    return reg;
  },
  amountNumber: function amountNumber(txt) {
    var type = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 6;
    var isRequired = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
    var regRule = type === 6 ? regExps.decimal_amount : regExps.decimal_price;
    var maxNumber = NUMBER[type];
    var max = maxNumber.max,
        message = maxNumber.message;
    var reg = [{
      validator: function validator(rule, value) {
  ...
  amountPrice: function amountPrice(txt, isRequired) {
